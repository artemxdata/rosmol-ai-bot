...
SPECIALIST_REDIRECT_RE = re.compile(
    r"(переда(?:ю|ем|ём|ть|йте)\s+(?:ваш\s+)?(?:вопрос|обращение)\s+специалист|"
    r"обрат(?:итесь|иться)\s+.*(?:служб[ау]\s+поддержк|специалист)|"
    r"рекомендую\s+(?:направить|обратиться)\s+.*(?:специалист|служб[ау]\s+поддержк))",
    flags=re.IGNORECASE,
)
