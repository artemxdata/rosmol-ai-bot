...
> **Актуализация 16 июля 2026.** Последующий gate был зелёным, но operator holdout прерван
> P0-компрометацией старой VM. Текущий статус — `NO GO / SECURITY HOLD`. Этот документ остаётся
> историческим аудитом качества; для текущих действий читать `docs/CURRENT_STATE.md`,
> `docs/security_incident_20260715.md` и `docs/operator_feedback_20260715.md`.
...
Исторический URL и SSH tunnel старой VM отозваны после P0-компрометации. До clean rebuild
доверенной админки нет. Новый HTTPS URL фиксируется в `CURRENT_STATE.md` только после отдельной
secret rotation, внешней проверки TLS и нового release handoff.
